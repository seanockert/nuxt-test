<template>
  <form
    ...
    method="get"
    :class="{ 'search-container': true, 'hidden-small': true, focused: active }"
    target="_blank"
    v-shortkey="['esc']"
        ...
        <ul v-else class="no-list">
          <li v-for="result in results" :key="result.id" ref="results">
            <a
              ...
              target="_blank"
            >
              <svg aria-hidden="true" height="16" width="16" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  fill-rule="nonzero"
            ...
            >
            <a v-else :href="result.url" title="View this resource" target="_blank">
              <figure>
...
export default {
  ...
  methods: {
    ...
    onSearchInput: debounce(function() {
      ...
      this.search(this.query, 'resource'); // replace with 'topic'
    }, 300),
    async search(query, type) {
      ...
      try {
        ...
        const response = await axios.get('https://api.teachstarter.com/v1/search/' + type, {
          params: {
            query: query,
            perPage: this.limit,
